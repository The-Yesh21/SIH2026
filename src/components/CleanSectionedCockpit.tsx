...
export function CleanSectionedCockpit({
  ...
  const [pythonBackendOnline, setPythonBackendOnline] = useState(false);
  ...
  useEffect(() => {
    ...
    probe();
    const interval = setInterval(probe, 6000);
  ...
  const handleSyncRealTime = () => {
    const now = new Date();
    setActiveClockMinutes(now.getHours() * 60 + now.getMinutes());
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-body selection:bg-blue-500 selection:text-white antialiased">
      
      {/* ========================================================================= */}
      {/* INDEPENDENT STANDALONE LIGHT MASTHEAD HEADER                              */}
      {/* ========================================================================= */}
      <header className="bg-white/95 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50 shadow-xs">
        <div className="w-full max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-10 py-3.5 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span>{pythonBackendOnline ? "Python ML Active" : "Hybrid Mode"}</span>
                </span>
              </div>
              <p className="text-xs text-slate-500 font-mono">
                SWR Corridor · <strong className="text-slate-800 font-bold">Mysuru (MYS) ➔ KSR Bengaluru (SBC)</strong> · 138.25 km
              </p>
            </div>
          </div>

          {/* Center Clock & Presets */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 bg-slate-100/80 p-1.5 rounded-xl border border-slate-200 font-mono text-xs">
            <div className="flex items-center justify-between sm:justify-start gap-2 px-2">
              ...
              >
                {isRealTimeSynced ? "● Live Sync" : "Sync Now"}
              </button>
            </div>

            {/* Quick Shift Presets */}
            <div className="flex items-center gap-1 border-t sm:border-t-0 sm:border-l border-slate-200 pt-1.5 sm:pt-0 sm:pl-2">
              {[
                { label: "Night", mins: 85 },
                { label: "Morning", mins: 450 },
                { label: "Midday", mins: 774 },
                { label: "Evening", mins: 1140 },
              ].map((p) => {
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              onClick={onSwitchToDarkCockpit}
              className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs transition-all shadow-sm"
            >
              <Moon className="w-3.5 h-3.5 text-amber-400" />
              <span>Dark Cockpit View</span>
            </button>
          </div>
        </div>
      </header>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search train by name or number (e.g., 12613, Wodeyar, Shatabdi, 16215)..."
                  value={searchQuery}
            ...
            {/* Responsive Train Cards Gallery Grid */}
      ...
      {/* Standalone Light Footer */}
      <footer className="border-t border-slate-200 bg-white py-4 px-6 text-center text-xs font-mono text-slate-500 mt-auto">
        RailRakshak Live Insight · SWR Mysore–Bangalore Division · Dynamic ML ETA, Corridor Bottleneck Forensics &amp; Movement Tracker
      </footer>
    </div>
  );
}
